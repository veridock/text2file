    ...
    try:
        ...
        try:
            ...
            if not result.is_valid:
                return (
                    False,
                    f"Validation failed for .{extension}: {result.message}",
                    generated_path,
                )
        except Exception as e:
            return (
                False,
                f"Error validating .{extension} file: {str(e)}",
                generated_path,
            )

        return (
            True,
            f"Successfully generated and validated .{extension} file",
            generated_path,
        )
...
def main():
    ...
    print(
        f"Testing all generators and validators. Output will be saved to: {output_dir}"
    )
    print("-" * 80)

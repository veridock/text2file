    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if result is not None:
                    logging.debug(
                        "Successfully parsed YAML with ruamel.yaml after cleaning"
                    )
                    return result
            except ImportError:
            ...
            try:
                ...
                if (
                    ...
                    and "nested:" in content
                    and "num: 42" in content
                ):
...
class ConfigGenerator(BaseGenerator):
    ...
    @classmethod
    def generate(
        cls, content: str, output_path: Union[str, Path], **kwargs: Any
    ) -> Path:
...
register_generator_directly(
    ["yaml", "yml"],
    lambda content, path, **kw: ConfigGenerator.generate(
        content, path, format="yaml", **kw
    ),
)
